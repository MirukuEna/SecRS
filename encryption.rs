 ...
 * Encryption Module.
 *
 * Implements high-grade data protection using AES-256-GCM (Galois/Counter Mode).
 * GCM is chosen for its authenticated encryption capabilities, ensuring both
 * confidentiality and integrity of the data.
 *
 * Key Management:
 * - Uses `tokio::sync::RwLock` for thread-safe key rotation.
 * - Keys are stored with metadata (creation/expiration) to enforce rotation policies.
 *
 * Author: KleaSCM
...
impl EncryptionManager {
    ...
    pub async fn encrypt(&self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        // Encrypt data
        // We use map_err because aes-gcm errors do not implement std::error::Error,
        // which breaks the `?` operator integration with anyhow::Result.
    ...
    pub async fn rotate_keys(&self) -> Result<()> {
        ...
        // Generate new key
        let new_key = self
            .generate_key(self.encryption_config.default_algorithm.clone())
            .await?;
    ...
    async fn save_key_to_storage(&self, key: &EncryptionKey) -> Result<()> {
        ...
        let key_path = format!(
            "{}/{}.key",
            self.encryption_config.key_storage_path, key.key_id
        );
