...
impl PermissionManager {
    ...
    pub async fn can_execute_command(&self, command: &str, user: &str) -> Result<bool> {
        ...
        if let Some(rule) = self.find_command_rule(command, user).await? {
            return Ok(self
                .check_rule_access(&rule, user, &user_permissions)
                .await?);
        }
    ...
    pub async fn can_access_network(
        &self,
        host: &str,
        port: u16,
        protocol: &str,
        ...
        if let Some(rule) = self
            .find_network_rule(&resource_path, protocol, user)
            .await?
        {
            return Ok(self
                .check_rule_access(&rule, user, &user_permissions)
    ...
    async fn find_file_rule(
        &self,
        path: &str,
        _operation: &str,
        user: &str,
    ) -> Result<Option<PermissionRule>> {
        let rules = self.rules.read().await;
    ...
    async fn check_rule_access(
        ...
        if !rule.allowed_roles.is_empty() {
            let has_role = user_permissions
                .roles
                .iter()
                .any(|role| rule.allowed_roles.contains(role));
            if !has_role {
                return Ok(false);
            }
        }
        ...
        if !rule.allowed_groups.is_empty() {
            let has_group = user_permissions
                .groups
                .iter()
                .any(|group| rule.allowed_groups.contains(group));
            if !has_group {
                return Ok(false);
            }
        }
        ...
        if !rule.denied_roles.is_empty() {
            let has_denied_role = user_permissions
                .roles
                .iter()
                .any(|role| rule.denied_roles.contains(role));
            if has_denied_role {
                return Ok(false);
            }
        }
        ...
        if !rule.denied_groups.is_empty() {
            let has_denied_group = user_permissions
                .groups
                .iter()
                .any(|group| rule.denied_groups.contains(group));
            if has_denied_group {
                return Ok(false);
            }
        }
    ...
    async fn check_ip_restrictions(&self, restrictions: &IpRestrictions) -> Result<bool> {
        ...
        if !restrictions.allowed_ranges.is_empty() {
            let in_allowed_range = restrictions
                .allowed_ranges
                .iter()
    ...
    async fn initialize_default_permissions(&self) -> Result<()> {
        ...
        roles.insert(
            "user".to_string(),
            HashSet::from(["file".to_string(), "network".to_string()]),
        );
        roles.insert("guest".to_string(), HashSet::from(["file".to_string()]));
