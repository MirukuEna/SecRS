...
impl AuditLogger {
    ...
    fn extract_source_info(
        ...
        match event {
            ...
            SecurityEvent::SecurityAlert { .. } => (Some("127.0.0.1".to_string()), None, None),
            SecurityEvent::MemoryAccess { pid, .. } => (
                Some("127.0.0.1".to_string()),
                None,
                Some(format!("pid_{}", pid)),
            ),
            SecurityEvent::NetworkPacket {
                source_ip, dest_ip, ..
            } => (Some(source_ip.clone()), Some(dest_ip.clone()), None),
            SecurityEvent::ThreatDetected { source, .. } => (Some(source.clone()), None, None),
    ...
    async fn start_background_tasks(&self) -> Result<()> {
        let _log_entries = self.log_entries.clone();
        let audit_config = self.audit_config.clone();
        let _log_writer = self.log_writer.clone();
                ...
                if let Ok(file) = std::fs::metadata(&audit_config.log_file_path) {
                    if file.len() > audit_config.max_file_size {
                        ...
                        if let Err(e) = Self::rotate_log_file(
                            &audit_config.log_file_path,
                            audit_config.max_log_files,
                        )
                        .await
                        {
    ...
    async fn update_event_counters(&self, entry: &AuditLogEntry) -> Result<()> {
        ...
        match &entry.event {
            SecurityEvent::SecurityAlert { severity, .. } => match severity {
    ...
    async fn check_alerts(&self, entry: &AuditLogEntry) -> Result<()> {
        ...
        if counters.critical_count >= thresholds.critical_per_minute {
            ...
            alert_message = format!(
                "Critical security events threshold exceeded: {} events per minute",
                counters.critical_count
            );
        } else if counters.high_count >= thresholds.high_per_minute {
            should_alert = true;
            alert_message = format!(
                "High severity security events threshold exceeded: {} events per minute",
                counters.high_count
            );
        } else if counters.medium_count >= thresholds.medium_per_minute {
            should_alert = true;
            alert_message = format!(
                "Medium severity security events threshold exceeded: {} events per minute",
                counters.medium_count
            );
        } else if counters.low_count >= thresholds.low_per_minute {
            should_alert = true;
            alert_message = format!(
                "Low severity security events threshold exceeded: {} events per minute",
                counters.low_count
            );
        }
        ...
        if should_alert {
            ...
            // Log alert manually to avoid recursion
            ...
            let alert_entry = self.create_log_entry(alert_event).await?;
            self.write_log_entry(&alert_entry).await?;
            // We do NOT call update_event_counters here to avoid double counting or infinite loops if counters trigger alerts
        }
    ...
    pub async fn get_entries_by_user(&self, user: &str) -> Vec<AuditLogEntry> {
        ...
        entries
            ...
            .filter(|entry| match &entry.event {
                ...
                SecurityEvent::BehavioralAnomaly {
                    user: event_user, ..
                } => event_user == user,
                _ => false,
            })
